...
assessment <- rbind(correlations %>% mutate(assessment="Spearman correlation") %>%
  ...
  mutate(metric = ifelse(metric=="r","Instantaneous growth rate",
                         ifelse(metric=="R","Effective reproduction number",
                                ifelse(metric%in%c("Instanaeous growth rate","Instantaeous growth rate"),
                                       "Instantaneous growth rate",metric))))
...
#           nrow=1
# )
# ggsave("outputs/analysis/Northern Ireland_heatmap.png",width=7,height=5.5)
# 
# 
# 
...
# 
# ggsave("outputs/analysis/Northern Ireland_metrics.png",width=6,height=5)


### maximal lags under the adjusted R2 for 40 knots

assessment %>% filter(knot_prop==0.4,
...
ons_area <- ons %>% filter(areaName=="Northern Ireland",
                           date<=as.Date("2022-12-28"))


panel_A <- ggplot()+
  theme_bw()+
  geom_ribbon(data=spline_fit_daily %>% filter(knot_prop==40),
              aes(x=date,ymin=Y.2.5.*100,ymax=Y.97.5.*100),alpha=0.6,fill="#077b8a")+
  geom_line(data=spline_fit_daily %>% filter(knot_prop==40),
  ...
  scale_y_continuous(breaks=seq(0,12,2),limits = c(0,11.5))
...
official_comb <- merge(estimates %>% filter(knot_prop==40),
                       official %>% filter(areaName=="Northern Ireland"),all.x=TRUE) %>% 
  mutate(transmissionRateGrowthRateMin.lag = Lag(transmissionRateGrowthRateMin,0),
         transmissionRateGrowthRateMax.lag = Lag(transmissionRateGrowthRateMax,0),
         transmissionRateGrowthRateMid.lag = Lag(transmissionRateGrowthRateMid,0),
         transmissionRateMin.lag = Lag(transmissionRateMin,9),
         transmissionRateMax.lag = Lag(transmissionRateMax,9),
         transmissionRateMid.lag = Lag(transmissionRateMid,9)) 
...
# ggsave("outputs/analysis/Northern Ireland.png",height=8,width=10)
...
assessment <- assessment %>% 
  mutate(metric_short = factor(ifelse(metric=="Effective reproduction number","R(t)",
                               ...
                               levels=c("R(t)","r(t)")),
         opt_lag = ifelse(assessment=="R2"&knot_prop==0.4,opt_lag,NA))
...
panel_F <- ggplot(assessment %>% filter(lag<=20,lag>-20,assessment=="R2",
  ...
  geom_vline(aes(xintercept=opt_lag,col=factor(knot_prop*100)),linetype="dotted")+
  labs(x="Lag (days)",
       y="R2",
       col="Knots (% of data points)",tag="F")+
...
ggsave("outputs/analysis/Northern Ireland.png",width=10,height=9)
...
plot_grid(plot_grid(panel_A + scale_x_date(date_breaks = "4 months",date_labels = "%b-%y"),
          ...
          plot_grid(panel_B+scale_x_date(date_breaks = "4 months",date_labels = "%b-%y",
                                         limits = c(as.Date("2020-09-17"),as.Date("2022-12-28"))),
...
ggsave("outputs/analysis/Northern Ireland_without_scatter.png",width=9,height=7)
...
plot_grid(panel_A,
          ...
          ncol=1,rel_heights=c(1,2))
ggsave("outputs/analysis/Northern Ireland_nolag.png",height=8,width=10)



### traceplot
fit <- readRDS("outputs/objects/fit_40_Northern Ireland.RDS")
traceplot(fit,pars=c("a[1]","a[13]","a[27]","a[41]","a[54]",
                     "Y_hat[1]","Y_hat[13]","Y_hat[27]","Y_hat[41]","Y_hat[54]",
                     "tau","gamma"))
ggsave("outputs/analysis/Northern Ireland_traceplots.png")
