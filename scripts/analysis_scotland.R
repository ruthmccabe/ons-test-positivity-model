...
#           nrow=1
# )
# ggsave("outputs/analysis/scotland_heatmap.png",width=7,height=5.5)
# 
# 
# 
...
# 
# ggsave("outputs/analysis/scotland_metrics.png",width=6,height=5)
...
ons_area <- ons %>% filter(areaName=="Scotland",
...
official_comb <- merge(estimates %>% filter(knot_prop==40),
                       official %>% filter(areaName=="Scotland")) %>% 
  mutate(transmissionRateGrowthRateMin.lag = Lag(transmissionRateGrowthRateMin,-11),
         transmissionRateGrowthRateMax.lag = Lag(transmissionRateGrowthRateMax,-11),
         transmissionRateGrowthRateMid.lag = Lag(transmissionRateGrowthRateMid,-11),
...
# ggsave("outputs/analysis/Scotland.png",height=8,width=10)
...
ggsave("outputs/analysis/Scotland.png",width=10,height=9)
...
plot_grid(plot_grid(panel_A + scale_x_date(date_breaks = "4 months",date_labels = "%b-%y"),
          ...
          plot_grid(panel_B+scale_x_date(date_breaks = "4 months",date_labels = "%b-%y",
                                         limits = c(as.Date("2020-10-09"),as.Date("2022-12-28"))),
...
ggsave("outputs/analysis/Scotland_without_scatter.png",width=9,height=7)
...
plot_grid(panel_A,
          ...
          ncol=1,rel_heights=c(1,2))
ggsave("outputs/analysis/Scotland_nolag.png",height=8,width=10)



### traceplot
fit <- readRDS("outputs/objects/fit_40_Scotland.RDS")
traceplot(fit,pars=c("a[1]","a[13]","a[26]","a[37]","a[52]",
                     "Y_hat[1]","Y_hat[13]","Y_hat[26]","Y_hat[37]","Y_hat[52]",
                     "tau","gamma"))
ggsave("outputs/analysis/Scotland_traceplots.png")
