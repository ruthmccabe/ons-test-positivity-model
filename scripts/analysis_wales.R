...
#           nrow=1
# )
# ggsave("outputs/analysis/Wales_heatmap.png",width=7,height=5.5)
...
# 
# ggsave("outputs/analysis/Wales_metrics.png",width=6,height=5)
# 
...
ons_area <- ons %>% filter(areaName=="Wales",
...
official_comb <- merge(estimates %>% filter(knot_prop==40),
                       official %>% filter(areaName=="Wales")) %>% 
  mutate(transmissionRateGrowthRateMin.lag = Lag(transmissionRateGrowthRateMin,-15),
         transmissionRateGrowthRateMax.lag = Lag(transmissionRateGrowthRateMax,-15),
         transmissionRateGrowthRateMid.lag = Lag(transmissionRateGrowthRateMid,-15),
         transmissionRateMin.lag = Lag(transmissionRateMin,-12),
         transmissionRateMax.lag = Lag(transmissionRateMax,-12),
         transmissionRateMid.lag = Lag(transmissionRateMid,-12)) 
...
# ggsave("outputs/analysis/Wales.png",height=8,width=10)
...
plot_grid(panel_A,
          ...
          ncol=1,rel_heights=c(1,2))
ggsave("outputs/analysis/Wales_nolag.png",height=8,width=10)



### traceplot
fit <- readRDS("outputs/objects/fit_40_Wales.RDS")
traceplot(fit,pars=c("a[1]","a[14]","a[28]","a[42]","a[56]",
                     "Y_hat[1]","Y_hat[14]","Y_hat[28]","Y_hat[42]","Y_hat[56]",
                     "tau","gamma"))
ggsave("outputs/analysis/Wales_traceplots.png")
